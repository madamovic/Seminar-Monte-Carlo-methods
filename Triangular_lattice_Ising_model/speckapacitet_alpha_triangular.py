...
input_file = pyalps.writeInputFiles('parmtriangular2',parms)
pyalps.runApplication('spinmc',input_file,Tmin=5)
# use the following instead if you have MPI
#pyalps.runApplication('spinmc',input_file,Tmin=5,MPI=4)

pyalps.evaluateSpinMC(pyalps.getResultFiles(prefix='parmtriangular2'))

#load the susceptibility and collect it as function of temperature T
data = pyalps.loadMeasurements(pyalps.getResultFiles(prefix='parmtriangular2'), 'Specific Heat')
...
plt.title(ur'2D Izingov model na trougaonoj rešetki, $\alpha/\nu=$ %.10s' % params[1])
plt.savefig("figure_alpha_over_nu_triangular.eps",dpi=300)
