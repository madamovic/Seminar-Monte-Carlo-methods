...
data = pyalps.loadMeasurements(pyalps.getResultFiles(prefix='parmtriangular'), 'Connected Susceptibility')
...
fh=open('susc_rounded_t_2D_Ising_triangular.txt','w')
...
s=open('susc_rounded_t_redosled_2D_Ising_triangular.txt','w')
...
for two_minus_eta in np.linspace(0.0,3.0,31):
  ...
  plt.savefig('figure_triangular_eta_procena%d.eps'%(numeratorfig),dpi=300)
  numeratorfig+=1
