...
for nth in np.arange(0,1001000,50000):
	for t in np.linspace(0.0,4.0,41):
		...
		input_file = pyalps.writeInputFiles('parmconven',parms)
		pyalps.runApplication('spinmc', input_file, Tmin=10, writexml=True)
		data = pyalps.loadMeasurements(pyalps.getResultFiles(prefix='parmconven'), 'Energy')
...
plt.legend(loc='best')
plt.savefig("konvergencija_en.eps",dpi=300)
