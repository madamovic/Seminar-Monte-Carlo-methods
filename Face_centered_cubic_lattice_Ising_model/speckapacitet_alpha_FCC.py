...
for l in [4,6,8,10,12,14,16,18]:
    for t in np.linspace(9.0,10.0,11):
        parms.append(
            { 
              'LATTICE'        : "fcc",
...
input_file = pyalps.writeInputFiles('parmFCCb',parms)
pyalps.runApplication('spinmc',input_file,Tmin=5)
# use the following instead if you have MPI
#pyalps.runApplication('spinmc',input_file,Tmin=5,MPI=4)

pyalps.evaluateSpinMC(pyalps.getResultFiles(prefix='parmFCCb'))

#load the susceptibility and collect it as function of temperature T
data = pyalps.loadMeasurements(pyalps.getResultFiles(prefix='parmFCCb'), 'Specific Heat')
...
plt.title(ur'3D Izingov model na površinski centriranoj kubnoj rešetki, $\alpha/\nu=$ %.10s' % params[1])
plt.savefig("figure_alpha_over_nu_FCC.eps",dpi=300)
