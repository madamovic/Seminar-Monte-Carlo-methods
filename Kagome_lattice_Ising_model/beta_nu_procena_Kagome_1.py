...
for l in [8,16,24,32,40]:
    for t in np.linspace(1.9,2.3,41):
        parms.append(
            { 
...
input_file = pyalps.writeInputFiles('parmkag',parms)
pyalps.runApplication('spinmc',input_file,Tmin=5)
# use the following instead if you have MPI
#pyalps.runApplication('spinmc',input_file,Tmin=5,MPI=4)

pyalps.evaluateSpinMC(pyalps.getResultFiles(prefix='parmkag'))

data = pyalps.loadMeasurements(pyalps.getResultFiles(prefix='parmkag'), '|Magnetization|')
...
fh=open('mag_rounded_t_2D_Ising_Kagome.txt','w')
...
s=open('mag_rounded_t_redosled_2D_Ising_Kagome.txt','w')
...
for beta_over_nu in np.linspace(0.0,1.6,161):
  ...
  plt.legend(loc='best')
  plt.savefig('figure_Kagome_beta_nu_binder_procena%d.eps'%(numeratorfig),dpi=300)
  numeratorfig+=1
