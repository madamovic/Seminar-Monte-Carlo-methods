...
file=open('binderdata_rounded_heisenberg_redosled_druga.txt')
...
if __name__ == '__main__':
    ...
    plt.xlabel(r'$J_{2}$')
    plt.ylabel(r'$U_{4}$')
    plt.savefig("presek_kumulanata_heisenbeg_binder_druga.eps",dpi=300)
    plt.show()

    for i in range(0,len(listaintersceptsx)):
		print(listaintersceptsx[i])

    textfile = open("kumulanti_binder_preseci_pojedinacni_druga.txt", "w")
