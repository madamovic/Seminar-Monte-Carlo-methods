...
data = pyalps.loadMeasurements(pyalps.getResultFiles(prefix='parm8b'),['Binder Ratio of Staggered Magnetization','Stiffness'])
...
plt.title(u'2D Hajzenbergov model, merdevinasti aranžman (coupled ladders)')
plt.legend(loc='best')
plt.savefig("stiffness_transition.eps",dpi=300)
...
plt.title(u'2D Hajzenbergov model, merdevinasti aranžman (coupled ladders)')
plt.legend(loc='best')
plt.savefig("binder_transition.eps",dpi=300)
...
s=open('binderdata_rounded_heisenberg_redosled.txt','w')
...
s=open('stiffness_rounded_heisenberg_redosled.txt','w')
