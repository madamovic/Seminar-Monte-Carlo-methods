...
data = pyalps.loadMeasurements(pyalps.getResultFiles(prefix='parmBCCa'),'Binder Cumulant')

numeratorfig=1

for tc in np.linspace(6.0,7.0,101):
    ...
    plt.title(u'3D Izingov model na zapreminski kubnoj rešetki')
    ...
    plt.savefig('figure_BCC_tc_binder_prva_procena%d.eps'%(numeratorfig),dpi=300)
    numeratorfig+=1