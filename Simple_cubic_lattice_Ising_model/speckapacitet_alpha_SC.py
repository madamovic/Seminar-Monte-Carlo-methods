...
for l in [4,6,8,10,12,14,16,18,20]:
    for t in np.linspace(4.0,5.0,21):
        parms.append(
            { 
...
input_file = pyalps.writeInputFiles('parmSC',parms)
pyalps.runApplication('spinmc',input_file,Tmin=5)
# use the following instead if you have MPI
#pyalps.runApplication('spinmc',input_file,Tmin=5,MPI=4)

pyalps.evaluateSpinMC(pyalps.getResultFiles(prefix='parmSC'))

#load the susceptibility and collect it as function of temperature T
data = pyalps.loadMeasurements(pyalps.getResultFiles(prefix='parmSC'), 'Specific Heat')
...
plt.title(ur'3D Izingov model na prostoj kubnoj rešetki, $\alpha/\nu=$ %.10s' % params[1])
plt.savefig("figure_alpha_over_nu_SC.eps",dpi=300)
